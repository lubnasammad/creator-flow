...
class AddPostingScreen extends StatefulWidget {
  final PostingSchedule? schedule;

  const AddPostingScreen({super.key, this.schedule});
...
class _AddPostingScreenState extends State<AddPostingScreen> {
  ...
  final _postingScheduleService = PostingScheduleService();
  ...
  final List<String> _statusOptions = ['Planned', 'Posted', 'Cancelled'];
  ...
  void initState() {
    ...
    if (widget.schedule != null) {
      ...
      _selectedDate = DateTime.parse(widget.schedule!.postDate);
      _platform = widget.schedule!.platform;
  ...
  Future<void> _handleSave() async {
    ...
    try {
      final schedule = PostingSchedule(
        ...
        postDate: DateFormat('yyyy-MM-dd').format(_selectedDate),
        platform: _platform,
        status: _status,
        createdAt:
      ...
      if (widget.schedule == null) {
        await _postingScheduleService.create(schedule);
      } else {
        await _postingScheduleService.update(schedule);
      }
